...
/// Unified genre-indicator list derived from `LCSH_FORM_GENRE_MAP`
/// plus common supplementary form keywords, cached once.
static GENRE_INDICATORS: LazyLock<Vec<String>> = LazyLock::new(|| {
    let mut indicators: Vec<String> = LCSH_FORM_GENRE_MAP.keys().map(|k| k.to_string()).collect();
    let extra = [
        "novel",
        "novels",
        "autobiography",
    ...
    for s in extra {
        ...
        if !indicators.contains(&s_owned) {
            indicators.push(s_owned);
        }
    }
    indicators
});

/// Counts occurrences of domain strings for frequency-based resolution.
fn count_frequencies<'a>(domains: &'a [&'a str]) -> HashMap<&'a str, usize> {
...
fn is_genre_keyword(s: &str) -> bool {
    ...
    let normalized = lower
        ...
        .collect::<Vec<_>>()
        .join(" ");
    for indicator in GENRE_INDICATORS.iter() {
        if normalized.contains(indicator.as_str()) {
            return true;
        }
    }
    false
...
pub fn extract_taxonomy(subjects_raw: &[String], bookshelves_raw: &[String]) -> Taxonomy {
    ...
    // Smart domain resolution: prefer LC-derived, weight by frequency/confidence.
    let lc_counts = count_frequencies(&lc_domains);
    let inferred_counts = count_frequencies(&inferred_domains);

    let primary_domain = if !lc_counts.is_empty() {
        // Prefer LC-derived domain with highest frequency; tie-break lexicographically.
        lc_counts
            .iter()
            .min_by(|a, b| b.1.cmp(a.1).then_with(|| a.0.cmp(b.0)))
            .map(|(dom, _)| *dom)
            .unwrap_or("General & Uncategorized")
    } else if !inferred_counts.is_empty() {
        // Fall back to most frequent inferred domain; tie-break lexicographically.
        inferred_counts
            .iter()
            .min_by(|a, b| b.1.cmp(a.1).then_with(|| a.0.cmp(b.0)))
            .map(|(dom, _)| *dom)
    ...
    // Deduplicate and format topics using normalized string keys.
    ...
    for parts in raw_topics {
        ...
        // Normalized key: lowercase, sorted subtopics, trimmed.
        let mut sub_sorted = subtopics.clone();
        sub_sorted.sort();
        let key = format!("{}|{}", heading.to_lowercase(), sub_sorted.join(","));
